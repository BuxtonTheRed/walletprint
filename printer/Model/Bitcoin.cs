...
namespace Casascius.Bitcoin {
    public class Util {
        ...
        public static byte[] ValidateAndGetHexPrivateKey(byte leadingbyte, string PrivHex, int desiredByteCount) {
            ...
            // if leading 00, change it to 0x80 - CORRECTION 0x9E for Dogecoin
            if (hex.Length == 33) {
                if (hex[0] == 0 || hex[0] == 0x9E) {
                    hex[0] = 0x9E;
                } else {
        ...
        public static string PubHashToAddress(string PubHash, string AddressType) {
            ...
            int cointype = 30; // changed default addressType for DOGE conversion
            if (Int32.TryParse(AddressType, out cointype) == false) cointype = 30;

            if (AddressType == "Testnet") cointype = 111;
            if (AddressType == "Namecoin") cointype = 52;
            if (AddressType == "Litecoin") cointype = 48;
            if (AddressType == "Dogecoin") cointype = 30;
