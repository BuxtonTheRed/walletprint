...
namespace Casascius.Bitcoin {
    ...
    public class KeyPair : PublicKey {
        ...
        /// </summary>
        public static KeyPair Create(string usersalt, bool compressed=false, byte addressType = 30) { // changed default addressType for DOGE conversion
        ...
        /// </summary>
        public KeyPair(BigInteger bi, bool compressed = false, byte addressType = 30) { // changed default addressType for DOGE conversion
        ...
        /// </summary>
        public KeyPair(byte[] bytes, bool compressed=false, byte addressType=30) { // changed default addressType for DOGE conversion
            if (bytes.Length == 32) {
        ...
        /// <summary>
        /// Create a Bitcoin address from a 32-byte private key - with WIF-stupid compatibility
        /// </summary>
        public KeyPair(byte[] bytes, byte addressType, bool isWIFstupid, bool compressed = false)
        { // changed default addressType for DOGE conversion
            if (bytes.Length == 32)
            {
                PrivateKeyBytes = bytes;
        ...
        /// <summary>
        /// Create a coin address from a key represented in a string.
        /// </summary>
        public KeyPair(string key, bool compressed=false, byte addressType=30) { // changed default addressType for DOGE conversion
            this._addressType = addressType;
            string result = constructWithKey(key, compressed, addressType);
        ...
        /// </summary>
        private string constructWithKey(string key, bool compressed, byte addressType = 30) {
            ...
            } else if (hex.Length == 33 && hex[0] == (0x80 + addressType)) {
                // normal private key
                _privKey = new byte[32];
                Array.Copy(hex, 1, _privKey, 0, 32);
                IsCompressedPoint = false;
            } else if (hex.Length == 34 && hex[0] == (0x80 + addressType) && hex[33] == 0x01) {
        ...
        public string PrivateKeyBase58 {
            get {
                ...
                if (IsCompressedPoint) {
                    ...
                    if (this._isWIFstupid)
                    {
                        rv[0] = 0x80;
                    }
                    else
                    {
                        rv[0] = (byte)(0x80 + AddressType);
                    }
            ...
            protected set {
                ...
                } else if (hex[0] != (this._isWIFstupid ? 0x80 : 0x80 + this._addressType) ) {
