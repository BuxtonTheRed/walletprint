...
namespace BtcAddress {
    ...
    class CoinInsert : System.Drawing.Printing.PrintDocument {
        ...
        protected Font fontsmaller = new Font("Courier New", 3.5F);
        ...
        protected override void OnPrintPage(System.Drawing.Printing.PrintPageEventArgs e) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                using (Pen blackpen = new Pen(Color.Black)) {
                    ...
                    // ADDITION for client - first 8 chars of address, in circle slightly overlapping the minikey one already generated
                    e.Graphics.DrawEllipse(blackpen, thiscodeX + 30F, thiscodeY + 10F + (CircleDiameterInches * 90F), CircleDiameterInches * 100F, CircleDiameterInches * 100F);
                    e.Graphics.FillEllipse(Brushes.White, thiscodeX + 30F, thiscodeY + 10F + (CircleDiameterInches * 90F), CircleDiameterInches * 100F, CircleDiameterInches * 100F);


                    e.Graphics.FillEllipse(Brushes.White, thiscodeX + 30F, thiscodeY + 10F, CircleDiameterInches * 100F, CircleDiameterInches * 100F);

                    // and then a quick horizontal line to mark where the key circle should be folded

                    e.Graphics.DrawLine(blackpen, thiscodeX + 20F, thiscodeY + (CircleDiameterInches * 118F), thiscodeX + 90F, thiscodeY + (CircleDiameterInches * 118F));
                ...
                // prep the repetitions of the first 8 chars of the Address
                string addressToRepeat = "";
                string startOfAddress = string.Format("++{0}++", address.Substring(0,8));
                for (int c = 0; c < 5; c++)
                {
                    addressToRepeat += (startOfAddress + "\r\n");
                }
                ...
                using (StringFormat sfcenter = new StringFormat()) {
                    ...
                    // print the first-8 chars-of-address pattern in to the second circle
                    e.Graphics.DrawString(addressToRepeat, fontsmaller, Brushes.Black, thiscodeX + 30F + (CircleDiameterInches * 100F / 2F), thiscodeY + 18.5F + (CircleDiameterInches * 90F), sfcenter);
                }
